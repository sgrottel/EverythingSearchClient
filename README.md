...
1. A brand-new named-pipe IPC with an accompanying "SDK v3" C API (`Everything3_*` functions). It's additive and powers advanced features (the property system, run-history, folder-size, live result-list monitoring, PCRE regex, viewport paging). Its wire protocol is **not publicly documented** — only the compiled C SDK wrapper is.
2. Incremental additions to the classic message-window IPC that this library speaks (new search flags, new sortable/queryable fields, a queue-rebuild command). These are fully documented and are what this library implements, starting with this release.

This library intentionally implements only (2), staying true to its goal of one managed AnyCPU DLL with no native SDK dependency. Using (1) would require either the native `Everything3.dll` or reverse-engineering an undocumented binary protocol. voidtools has confirmed that previous IPC versions remain compatible with Everything 1.5, so this library continues to work against 1.3, 1.4, and 1.5 installs unchanged, gaining the new 1.5 fields where the installed version supports them.
